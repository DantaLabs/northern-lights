...
func NewDemoTransport() *DemoTransport {
	var fixture SheetData
	if err := json.Unmarshal(demoSheetdataJSON, &fixture); err != nil {
...
// values, data update, and operations endpoints used by the Workiva client.
func (t *DemoTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	case isDataPath(path) && method == http.MethodPatch:
		return t.demoUpdateResponse(path), nil
...
// isDataPath reports whether path is a sheet data PATCH endpoint.
...
// demoUpdateResponse records the simulated write and returns 202 Accepted.
func (t *DemoTransport) demoUpdateResponse(path string) *http.Response {
	body := []byte(`{"operationLocation":"/operations/demo-op-1","id":"demo-op-1"}`)
	t.mu.Lock()
	t.WrittenCells[path] = "updated"
	t.mu.Unlock()
	return &http.Response{
		StatusCode: http.StatusAccepted,
		Header:     http.Header{"Content-Type": []string{"application/json"}},
		Body:       io.NopCloser(bytes.NewReader(body)),
		Request:    &http.Request{Method: http.MethodPatch, URL: &url.URL{Path: path}},
	}
}

// demoOperationCompletedJSON returns a completed async operation.
func demoOperationCompletedJSON() []byte {
	return []byte(`{"id":"demo-op-1","status":"completed","resourceUrl":"/spreadsheets/demo-sp-1/sheets/demo-sh-b3-energy/data"}`)
}
